...
const tools=[
  ...
  ["get_correlation","assetA, assetB","Correlation of DreamDEX log returns plus interval, sample and confidence metadata."],
...
export default function Reference(){return <><SiteNav active="reference"/><main className="wrap page-pad docs"><div className="eyebrow-plain">MCP reference</div><h1 className="page-title">Six tools, one CrossOdds engine</h1><p className="page-intro">The sentiment and divergence tools remain backward compatible; correlation and basket pricing extend the same MCP server used by the CrossOdds web app.</p>{tools.map(([name,input,desc])=><div className="tool-card" key={name}><div className="tool-head"><span className="tool-name">{name}</span><span className="tool-tag">{input}</span></div><p>{desc}</p></div>)}<h2>HTTP API</h2><div className="code-block">GET /health<br/>GET /api/assets<br/>GET /api/sentiment/BTC<br/>GET /api/divergence/BTC<br/>GET /api/market/BTC<br/>GET /api/correlation?assetA=BTC&amp;assetB=ETH<br/>POST /api/basket/price</div></main><SiteFoot/></>}
