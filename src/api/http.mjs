...
const safeWindow = (value) => {
  ...
  return Number.isFinite(number) && number > 0 && number <= 1440 ? number : NaN;
};
...
const server = http.createServer(async (req, res) => {
  ...
  try {
    ...
    if (req.method === "GET" && url.pathname === "/health") return json(res, 200, { ok: true, service: "crossodds", version: VERSION, mode: process.env.DEMO_MODE === "true" ? "demo" : "live" });
    ...
    if (req.method === "GET" && url.pathname === "/api/correlation") {
      ...
      const marketWindowMin = safeWindow(url.searchParams.get("marketWindowMin"));
      if (Number.isNaN(marketWindowMin)) return json(res, 400, { error: "invalid marketWindowMin" });
      return json(res, 200, await calculateAssetCorrelation(assetA, assetB, { preferredMarketWindowMin: marketWindowMin }));
    }
    ...
    if (req.method === "POST" && url.pathname === "/api/basket/price") {
      ...
      const legs = body.legs.map((leg) => ({ asset: safeAsset(leg.asset), direction: String(leg.direction || "UP").toUpperCase(), intervalMin: safeWindow(leg.intervalMin) }));
      if (legs.some((leg) => !leg.asset || !["UP", "DOWN"].includes(leg.direction) || Number.isNaN(leg.intervalMin))) return json(res, 400, { error: "invalid basket leg" });
      const requestedWindows = legs.map((leg) => leg.intervalMin).filter((value) => value !== null);
      if (requestedWindows.length === 1 || (requestedWindows.length === 2 && requestedWindows[0] !== requestedWindows[1])) return json(res, 400, { error: "basket legs must use the same event window" });
...
server.listen(PORT, HOST, () => console.log(`CrossOdds HTTP API listening on http://${HOST}:${PORT}`));
